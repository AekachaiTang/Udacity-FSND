...
def create_app(test_config=None):
    ...
    def paginate_result(request, section):
        page = request.args.get('page', 1, type=int)
        start = (page - 1) * ROWS_PER_PAGE
        end = start + ROWS_PER_PAGE

        object_formatted = [object_name.format() for object_name in section]
        return object_formatted[start:end]
    ...
    def get_actors(payload):
        ...
        actors_paginated = paginate_result(request, selection)
    ...
    def get_movies(payload):
        ...
        movies_paginated = paginate_result(request, selection)
    ...
    def delete_movies(payload, movie_id):
        ...
        movie_to_delete = Movie.query.filter(Movie.id == movie_id).one_to_none()
